...
class TestGetDefaults:
    def test_returns_hardcoded_defaults_when_none_set(self, initialized_cli):
        defaults = get_defaults(initialized_cli / ".yait")
    ...
    def test_returns_custom_values(self, initialized_cli):
        cfg = _read_config(initialized_cli / ".yait")
        cfg["defaults"] = {"type": "bug", "priority": "p2"}
        _write_config(initialized_cli / ".yait", cfg)
        defaults = get_defaults(initialized_cli / ".yait")
...
class TestGetDisplay:
    def test_returns_hardcoded_defaults_when_none_set(self, initialized_cli):
        display = get_display(initialized_cli / ".yait")
...
class TestGetConfigValue:
    def test_get_defaults_type(self, initialized_cli):
        assert get_config_value(initialized_cli / ".yait", "defaults.type") == "misc"

    def test_get_display_max_title_width(self, initialized_cli):
        assert get_config_value(initialized_cli / ".yait", "display.max_title_width") == 50

    def test_invalid_key_no_dot(self, initialized_cli):
        with pytest.raises(KeyError, match="section.field"):
            get_config_value(initialized_cli / ".yait", "nope")

    def test_unknown_section(self, initialized_cli):
        with pytest.raises(KeyError, match="Unknown config section"):
            get_config_value(initialized_cli / ".yait", "foo.bar")

    def test_unknown_field(self, initialized_cli):
        with pytest.raises(KeyError, match="Unknown config key"):
            get_config_value(initialized_cli / ".yait", "defaults.foo")


class TestSetConfigValue:
    def test_set_defaults_type(self, initialized_cli):
        set_config_value(initialized_cli / ".yait", "defaults.type", "bug")
        assert get_config_value(initialized_cli / ".yait", "defaults.type") == "bug"

    def test_set_display_max_title_width(self, initialized_cli):
        set_config_value(initialized_cli / ".yait", "display.max_title_width", "80")
        assert get_config_value(initialized_cli / ".yait", "display.max_title_width") == 80

    def test_set_max_title_width_non_int_raises(self, initialized_cli):
        with pytest.raises(ValueError, match="must be an integer"):
            set_config_value(initialized_cli / ".yait", "display.max_title_width", "abc")

    def test_set_defaults_labels(self, initialized_cli):
        set_config_value(initialized_cli / ".yait", "defaults.labels", "bug,urgent")
        assert get_config_value(initialized_cli / ".yait", "defaults.labels") == ["bug", "urgent"]

    def test_set_defaults_labels_empty(self, initialized_cli):
        set_config_value(initialized_cli / ".yait", "defaults.labels", "")
        assert get_config_value(initialized_cli / ".yait", "defaults.labels") == []

    def test_set_defaults_assignee(self, initialized_cli):
        set_config_value(initialized_cli / ".yait", "defaults.assignee", "alice")
        assert get_config_value(initialized_cli / ".yait", "defaults.assignee") == "alice"

    def test_set_defaults_assignee_null(self, initialized_cli):
        set_config_value(initialized_cli / ".yait", "defaults.assignee", "alice")
        set_config_value(initialized_cli / ".yait", "defaults.assignee", "null")
        assert get_config_value(initialized_cli / ".yait", "defaults.assignee") is None

    def test_set_unknown_key_raises(self, initialized_cli):
        with pytest.raises(KeyError):
            set_config_value(initialized_cli / ".yait", "defaults.foo", "bar")
...
class TestResetConfigValue:
    def test_reset_removes_from_yaml(self, initialized_cli):
        set_config_value(initialized_cli / ".yait", "defaults.type", "bug")
        reset_config_value(initialized_cli / ".yait", "defaults.type")
        assert get_config_value(initialized_cli / ".yait", "defaults.type") == "misc"
        cfg = yaml.safe_load((initialized_cli / ".yait" / "config.yaml").read_text())
    ...
    def test_reset_unknown_key_raises(self, initialized_cli):
        with pytest.raises(KeyError):
            reset_config_value(initialized_cli / ".yait", "foo.bar")
...
class TestNewWithConfigDefaults:
    ...
    def test_new_uses_default_priority(self, runner, initialized_cli):
        set_config_value(initialized_cli / ".yait", "defaults.priority", "p2")
        result = runner.invoke(main, ["new", "--title", "Test"], catch_exceptions=False)
        assert result.exit_code == 0
        issue = load_issue(initialized_cli / ".yait", 1)
    ...
    def test_new_uses_default_labels(self, runner, initialized_cli):
        set_config_value(initialized_cli / ".yait", "defaults.labels", "triage,review")
        result = runner.invoke(main, ["new", "--title", "Test"], catch_exceptions=False)
        assert result.exit_code == 0
        issue = load_issue(initialized_cli / ".yait", 1)
    ...
    def test_template_overrides_config_defaults(self, runner, initialized_cli):
        ...
        set_config_value(initialized_cli / ".yait", "defaults.type", "enhancement")
        set_config_value(initialized_cli / ".yait", "defaults.priority", "p3")
        save_template(initialized_cli / ".yait", Template(
